...
def _make_retrieval_coco_karpathy_dataset_index(
    ...
    # 初始化一个列表，初始化一个集合imagecounter用以统计唯一的图像数量
    items = []
    image_counter = set()
...
class SingleItemDataset(torch.utils.data.Dataset):
    def __init__(self, args, item_id, transform=None, tokenizer=None):
        self.args = args  # 必须作为第一个参数接收
        self.item_id = item_id
        self.transform = transform
        self.tokenizer = tokenizer
        self.data = self._load_specific_item(item_id)

    def _load_specific_item(self, item_id):
        # 构建完整图片路径（兼容子目录）
        img_path = os.path.join(self.args.data_path, item_id)
    ...
    def __getitem__(self, idx):
        img = Image.open(self.data['image_path']).convert('RGB')
        if self.transform:
            img = self.transform(img)
        return {
            'image': img,
            'image_id': self.data['image_id'],
            'text': self.tokenizer.encode(self.data['caption']) if self.tokenizer else ""
        }
