...
def get_args():
    ...
    parser.add_argument("--single_item_id", type=str, default=None,
                   help="Evaluate a specific item by ID (e.g., 'COCO_val2014_000000123')")
...
def main(args, ds_init):
    ...
    # 计算实际的总批次大小和每个epoch的训练步数
    """total_batch_size：

        args.batch_size：单GPU的批次大小。

        args.update_freq：梯度累积步数（模拟更大批次）。

        utils.get_world_size()：分布式训练中的GPU数量。
    """
    ...
    # 任务头（Task Head）特殊学习率​
        ...
        # 分类头的学习率倍数 通过函数get_is_head_flag_for_vit识别分类头参数
    ...
    # 当不使用DeepSpeed时，用PyTorch原生DDP包裹模型。
    else:
        if args.distributed:
