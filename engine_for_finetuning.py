...
class RetrievalHandler(TaskHandler):
    def __init__(self) -> None:
        ...
        # 新增：预加载的特征库
        self.image_feat_db = None  
        self.text_feat_db = None

    def load_feature_database(self, image_feats, text_feats):
        """预加载特征库用于快速检索"""
        self.image_feat_db = image_feats  # [N, D]
        self.text_feat_db = text_feats   # [M, D]
    ...
    # ========== 推理相关方法 ==========
    def before_infer(self, **kwargs):
        self.image_feats = []  # 存储所有batch的图像特征
        self.text_feats = None  # 初始为None，表示尚未计算文本特征
        self.image_path = []

    def infer_batch(self, model, image, language_tokens, padding_mask,image_path):
        # 只在第一次调用时计算文本特征
        if self.text_feats is None:
            ...
            # 强制类型转换和验证
            if isinstance(language_cls, list):
                language_cls = language_cls[0]  # 取第一个元素（假设需要单个特征）
            
            
            self.text_feats = language_cls.clone().detach()  # 确保独立张量
        # 计算当前batch的图像特征
        with torch.no_grad():
    ...
    def after_infer(self, **kwargs):
        # 合并所有图像特征
        image_cls_feats = torch.cat(self.image_feats, dim=0)  # [N, D]

        # 确保文本特征是2D的 [1, D]
        if len(self.text_feats.shape) == 1:
            text_cls_feats = self.text_feats.unsqueeze(0)
        else:
            text_cls_feats = self.text_feats
        
        # 归一化特征
        image_cls_feats = F.normalize(image_cls_feats, dim=1)
        text_cls_feats = F.normalize(text_cls_feats, dim=1)
        
        # 计算相似度矩阵 [N, 1]
        scores = torch.matmul(image_cls_feats, text_cls_feats.t())
        
        print(f"相似度矩阵形状: {scores.shape}")
        print(f"示例相似度值: {scores[:5].squeeze().tolist()}")  # 打印前5个相似度值
        
        return {
            "image_features": image_cls_feats,  # [N, D]
            "text_features": text_cls_feats,    # [1, D]
            "similarity_scores": scores,         # [N, 1]
    ...
    def eval_batch(self, model, image, language_tokens, padding_mask, image_id):
        ...
        print("Processed Image IDs:", image_id.cpu().numpy().tolist())  # 输出当前batch的所有image_id
    
    # 如果需要更详细的输出，可以添加以下内容：
        print(f"Batch Summary:")
        print(f"  Image features shape: {vision_cls.shape}")
        print(f"  Text features shape: {language_cls.shape}")
        print(f"  Number of samples: {image_id.shape[0]}")
...
class CaptioningHandler(TaskHandler):
    ...
    def eval_batch(self, model, image, image_id=None):
        ...
        if image_id is not None:
            print("Retrieved Image IDs:", [img_id.item() for img_id in image_id])
...
def train_one_epoch(
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        #  前向传播与损失计算​
        if mixup_fn is not None:# MixUp/CutMix数据增强
...
def evaluate(data_loader, model, device, handler):
    # 创建一个MetricLogger对象用于记录和统计评估指标 
    # delimiter="  "设置指标之间的分隔符为两个空格
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'

    # switch to evaluation mode
    model.eval()
    # 调用handler的before_eval方法，执行评估前的准备工作
